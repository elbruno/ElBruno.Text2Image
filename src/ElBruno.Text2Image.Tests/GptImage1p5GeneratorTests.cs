...
/// <summary>
/// Test helper: Mocks the Azure.AI.OpenAI ImageClient for unit testing.
/// Allows full control over response behavior without real Azure API calls.
/// </summary>
internal interface IImageClientForTesting
{
...
/// <summary>
/// Represents generated image result for testing.
/// Mirrors Azure.AI.OpenAI.GeneratedImage structure.
/// </summary>
internal class GeneratedImageForTesting
{
...
/// <summary>
/// Represents image generation options for testing.
/// Mirrors Azure.AI.OpenAI.ImageGenerationOptions structure.
/// </summary>
internal class ImageGenerationOptionsForTesting
{
...
/// <summary>
/// Fake ImageClient for unit testing. Allows test control over response behavior.
/// </summary>
internal sealed class FakeImageClientForTesting : IImageClientForTesting
{
...
public class GptImage1p5GeneratorConstructorValidationTests
{
    [Fact]
    public void Constructor_NullEndpoint_Throws()
    {
        var ex = Assert.ThrowsAny<ArgumentException>(() =>
            TestableGptImage1p5Generator.Create(endpoint: null!, apiKey: "key", deploymentName: "deploy"));
    ...
    {
        var ex = Assert.ThrowsAny<ArgumentException>(() =>
            TestableGptImage1p5Generator.Create(endpoint: "", apiKey: "key", deploymentName: "deploy"));
        Assert.Contains("endpoint", ex.Message, StringComparison.OrdinalIgnoreCase);
    }
    ...
    {
        var ex = Assert.ThrowsAny<ArgumentException>(() =>
            TestableGptImage1p5Generator.Create(
    ...
    {
        var generator = TestableGptImage1p5Generator.Create(
            endpoint: "https://example.services.ai.azure.com",
            apiKey: "test-key",
            deploymentName: "gpt-image-15");

        Assert.NotNull(generator);
        Assert.Equal("GPT-Image-1.5", generator.ModelName);
    }
}
...
public class GptImage1p5GeneratorPromptValidationTests
{
    [Fact]
...
public class GptImage1p5GeneratorSizeTests
{
    [Fact]
    ...
    public async Task GenerateAsync_Size512x512_MapsTo1024x1024()
    {
        ...
        Assert.NotNull(result);
        // Should be mapped to 1024x1024
        Assert.Equal(1024, result.Width);
        Assert.Equal(1024, result.Height);
    }

    [Fact]
    public async Task GenerateAsync_Size2000x2000_MapsTo1024x1024()
    {
        var generator = TestableGptImage1p5Generator.Create(
            "https://example.services.ai.azure.com", "key", "deploy");
        ...
        Assert.NotNull(result);
        // Should be mapped to largest available size
        Assert.True(result.Width <= 1792 && result.Height <= 1792);
    }

    [Fact]
    public async Task GenerateAsync_Size1600x1200_MapsTo1792x1024Landscape()
    {
        var generator = TestableGptImage1p5Generator.Create(
            "https://example.services.ai.azure.com", "key", "deploy");
        ...
        Assert.NotNull(result);
        // Should be mapped to landscape (1792x1024)
        Assert.Equal(1792, result.Width);
        Assert.Equal(1024, result.Height);
    }
...
public class GptImage1p5GeneratorRequestResponseTests
{
    [Fact]
    ...
    [Fact]
    public async Task GenerateAsync_DeploymentNameInResult()
    {
        var generator = TestableGptImage1p5Generator.Create(
            "https://example.services.ai.azure.com", "key", "gpt-image-15-deploy");

        var result = await generator.GenerateAsync("test");

        Assert.NotNull(result);
        // Deployment name should be accessible (in metadata or dedicated property)
    }
...
public class GptImage1p5GeneratorErrorHandlingTests
{
    [Fact]
    ...
    [Fact]
    public async Task GenerateAsync_NetworkError_RetryGuidance()
    {
        var generator = TestableGptImage1p5Generator.Create(
            "https://example.services.ai.azure.com", "key", "deploy",
    ...
    [Fact]
    public async Task GenerateAsync_TimeoutError_LogsAndThrows()
    {
        var generator = TestableGptImage1p5Generator.Create(
            "https://example.services.ai.azure.com", "key", "deploy",
    ...
    [Fact]
    public async Task GenerateAsync_RateLimitedError_ClearMessage()
    {
        var generator = TestableGptImage1p5Generator.Create(
            "https://example.services.ai.azure.com", "key", "deploy",
    ...
    [Fact]
    public async Task GenerateAsync_MalformedEndpointUri_ThrowsBeforeCall()
    {
        var ex = Assert.ThrowsAny<ArgumentException>(() =>
            TestableGptImage1p5Generator.Create("not-a-valid-uri", "key", "deploy"));
        Assert.NotNull(ex);
    }
}
...
public class GptImage1p5GeneratorLoggingTests
{
    [Fact]
    public async Task GenerateAsync_GenerationStartLogged()
    {
        ...
        // Verify generation start was logged
    ...
    public async Task GenerateAsync_SizeMappingLogged()
    {
        ...
        // Size mapping should be logged if mapping occurred
    }
}
...
/// <summary>
/// Testable wrapper for GptImage1p5Generator that allows dependency injection for testing.
/// </summary>
internal sealed class TestableGptImage1p5Generator : IImageGenerator
{
    private readonly string _endpoint;
    private readonly string _apiKey;
    private readonly string _deploymentName;
    private readonly IImageClientForTesting _imageClient;
    private readonly ILogger? _logger;
    private readonly Exception? _throwOnGenerate;

    public string ModelName => "GPT-Image-1.5";
    public string ModelId => "gpt-image-15";
    public string Endpoint => _endpoint;

    private TestableGptImage1p5Generator(
        string endpoint,
        string apiKey,
        string deploymentName,
        IImageClientForTesting imageClient,
        ILogger? logger = null,
    ...
    public static TestableGptImage1p5Generator Create(
        string endpoint,
        string apiKey,
        string deploymentName,
        IImageClientForTesting? imageClient = null,
        ILogger? logger = null,
        Exception? throwOnGenerate = null)
    {
        // Validation
        ...
        imageClient ??= new FakeImageClientForTesting();

        return new TestableGptImage1p5Generator(endpoint, apiKey, deploymentName, imageClient, logger, throwOnGenerate);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        _logger?.LogInformation(
            "Generating image with GPT-Image-1.5: prompt length={PromptLength}, size={Size}",
            prompt.Length,
...
/// <summary>
/// Test logger implementation to capture log output.
/// </summary>
internal sealed class TestLogCapture : ILogger
{
