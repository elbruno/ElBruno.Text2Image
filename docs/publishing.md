...
This guide covers how to publish new versions of **ElBruno.Text2Image** to NuGet.org using GitHub Actions and NuGet Trusted Publishing (keyless, OIDC-based).

## Package

| Package | Project | Description |
|---------|---------|-------------|
| `ElBruno.Text2Image` | `src/ElBruno.Text2Image/` | Text-to-image generation using Stable Diffusion (ONNX) and FLUX.2 (cloud API) |
...
2. **NuGet icon source** (already configured):
   ...
   - `images/logo_01_nuget.png`
   - Packed into the `.nupkg` as `logo_01_nuget.png` via `<PackageIcon>logo_01_nuget.png</PackageIcon>`
...
GitHub Release created (e.g. v1.0.0)
  → GitHub Actions triggers publish.yml
    ...
    → Packs ElBruno.Text2Image.nupkg
...
3. **csproj fallback** — reads `<Version>` from `src/ElBruno.Text2Image/ElBruno.Text2Image.csproj`
...
- [ElBruno.Text2Image on NuGet.org](https://www.nuget.org/packages/ElBruno.Text2Image) — The published package page (once published)
